...
namespace Alligator.Structural.Loads
{
    public class CreateNodalLoad : BHoMBaseComponent<BHL.Load<BHE.Node>>
    {
        ...
        private bool GetNodesOrGroup(IGH_DataAccess DA, int index, out  BHB.Group<BHE.Node> nodes)
        {
            ...
            if (goObjs[0].Value is BHB.Group<BHE.Node>)
            {
                nodes = (BHB.Group<BHE.Node>)goObjs[0].Value;
                for (int i = 1; i < goObjs.Count; i++)
                {
                    nodes.Data.AddRange((List<BHE.Node>)(BHB.Group<BHE.Node>)goObjs[i].Value);
                }
                return true;
            }

            nodes = new BHB.Group<BHE.Node>();

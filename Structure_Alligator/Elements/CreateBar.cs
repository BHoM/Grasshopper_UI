...
namespace BH.UI.Alligator.Structural.Elements
{
    ...
    public class CreateBar : GH_Component
    {
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            DA.SetData(0, bar);
            DA.SetData(1, GHE.GeometryUtils.Convert(bar.GetCentreline()));
        }
        ...
        private bool GetEndNodes(IGH_DataAccess DA, int firstDAindex, out BHE.Node stNode, out BHE.Node enNode)
        {
            ...
            if (typeof(BHG.ICurve).IsAssignableFrom(ncl.GetType()))
            {
                stNode = new BHE.Node(((BHG.ICurve)ncl).GetStartPoint());
                enNode = new BHE.Node(((BHG.ICurve)ncl).GetEndPoint());
                return true;
            }

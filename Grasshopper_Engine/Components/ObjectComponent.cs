    ...
    for (int i = 0; i < propInfo.Length; i++)
    {
        ...
        if (propInfo[i].GetSetMethod() != null && GHE.DataUtils.IsBrowseable(propInfo[i]))
        {
            ...
            else if (GHE.DataUtils.IsGeometric(pType))
            {
                ...
                else if (pType == typeof(BH.oM.Geometry.Curve))
                {
                    pManager.AddCurveParameter(name, nickName, description, access);
                }
                else if (pType == typeof(BH.oM.Geometry.Surface))
                {
                    pManager.AddBrepParameter(name, nickName, description, access);
                }
                else if (typeof(BH.oM.Geometry.IGroup).IsAssignableFrom(pType))
                {
...
protected override void RegisterOutputParams(GH_OutputParamManager pManager)
{
    ...
    for (int i = 0; i < propInfo.Length; i++)
    {
        ...
        {
            if (typeof(BH.oM.Geometry.GeometryBase).IsAssignableFrom(pType))//.BaseType == typeof(BH.oM.Geometry.GeometryBase))
            {
                string name = GHE.DataUtils.GetName(propInfo[i]);
...
protected override void SolveInstance(IGH_DataAccess DA)
{
    ...
    for (int i = 0; i < Params.Input.Count; i++)
    {
        ...
        if (Params.Input[i].Access == GH_ParamAccess.item)
        {
            ...
            else if (pType.BaseType == typeof(BH.oM.Geometry.GeometryBase))
            {
        ...
        else
        {
            if (pType.IsGenericType)
            {
                ...
                if (listType.BaseType == typeof(BH.oM.Geometry.GeometryBase))
                {
...
protected void SetGeometry(object obj, IGH_DataAccess DA)
{
    ...
    {
        Type pType = propInfo[i].PropertyType;
        if (propInfo[i].GetGetMethod() != null && typeof(BH.oM.Geometry.GeometryBase).IsAssignableFrom(pType))
        {
            ...
            {
                DA.SetData(geomIndex++, GHE.GeometryUtils.Convert(propInfo[i].GetValue(obj, null) as BH.oM.Geometry.GeometryBase));
            }
        }
    }
}
